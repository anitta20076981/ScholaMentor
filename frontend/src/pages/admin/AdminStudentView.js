  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/admin/view-student/${studentId}`);
        setSponsor(res.data);
...
const handleAction = async (action) => {
    ...
    try {
      ...
      const endpoint =
        action === "approve"
          ? `/api/admin/approve-sponsor/${studentId}/approve`
          : '';
    ...
    <AdminSidebar>
      ...
      {loading ? (
        ...
        <section className="recommend-section view-request-section">
            ...
            <p><strong>Pincode:</strong> {students.pincode}</p>
            
            </div>

            {/* Application Details */}
            <div className="request-column">
            <h3 className="section-title">School Info/ College Info</h3>
            <p><strong>School or College:</strong> {students.school_or_college	}</p>
            <p><strong>Course:</strong> {students.course}</p>
            <p><strong>Department:</strong> {students.department}</p>
            <p><strong>Year:</strong> {students.year}</p>
            <p><strong>Tution Fee:</strong> {students.tuition_fee}</p>
            <p>
                <strong>Id Proof:</strong> {renderDocLink(":", students.id_proof)}
            </p><p>
                <strong>Address Proof:</strong> {renderDocLink(":", students.address_proof)}
            </p>
            </div>  
